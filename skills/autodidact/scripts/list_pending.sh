...
OUTPUT=$(python "$SCRIPT_DIR/pending.py" list)

if [[ -n "$OUTPUT" ]]; then
    echo "autodidact: pending proposals awaiting approval (id / action / target / summary):"
    echo "$OUTPUT"
    echo "Approve with: python .claude/skills/autodidact/scripts/pending.py approve <id>"
    echo "Reject with: python .claude/skills/autodidact/scripts/pending.py reject <id>"
fi
