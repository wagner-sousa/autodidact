...
Once fired, the hook calls `pending.py new --action create --target <domain>` itself — staging a manifest is content-free and safe to do without user confirmation — then prints an instruction telling the agent to use `AskUserQuestion` on the staged id so the user decides to approve, reject, or defer. If an entry for the target already exists, the hook reuses it rather than staging a duplicate. Generating actual `SKILL.md` content still requires domain knowledge only skill-creator has and happens only after approval. The full flow: hook stages request + tells agent to ask -> user approves -> agent invokes `skill-creator`, which drafts the real `SKILL.md` directly under `.claude/skills/<domain>/` -> `detect_pending_completion.py` clears the entry once it lands.
