...
- `detect_complexity.py` — Stop hook. Reads the hook JSON payload from stdin, needs `transcript_path` (path to the JSONL conversation transcript). Always prints an unconditional English reminder covering four criteria: (a) tool-call volume vs the configured `min_tool_calls` threshold, (b) an error or dead end worked around, (c) the user correcting the approach, (d) a non-obvious discovery. The reminder includes this turn's actual counts (tool calls, file edits, file reads) and the `config.json` trigger thresholds so the agent can see exactly how far it is from the threshold without reading the file. Also counts `tool_use` blocks; if >= threshold, additionally writes `.state/pending.json` for a richer nudge at the start of the next turn. The real trigger, either way, is the agent's own end-of-task judgment (see `SKILL.md`).
...
- `detect_staleness.py` — Stop hook. Reads the whole session transcript (not just the last turn) for `Skill` tool invocations and `Edit`/`Write`/`NotebookEdit` file paths. Maps edited paths to known domains (same `known_domains` config as `detect_domain_recurrence.py`); if a domain's files were touched, its skill directory exists, but that skill was never invoked via the `Skill` tool this session, stages a `patch` request and instructs the agent to ask via `AskUserQuestion` at the end of the task — a signal the skill may be missing something, not a signal to create a new one.
