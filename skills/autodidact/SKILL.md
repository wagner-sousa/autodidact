...
3. **The domain-recurrence nudge (stages the request itself).** `scripts/detect_domain_recurrence.py` watches for the same uncovered domain coming up repeatedly across separate prompts — a pattern `detect_complexity.py` misses because no single turn crosses its tool-call bar. It never generates skill content — that stays skill-creator's job — but once a domain hits `min_mentions` it calls `pending.py new --action create --target <domain>` itself (staging a manifest is content-free and safe to do without confirmation) and prints an instruction telling the agent to use `AskUserQuestion` on the resulting id at the end of the current task, not mid-task. A printed-only reminder is too easy to miss across a whole new session; an already-staged, already-numbered request is not. It fires on every subsequent mention (not just the first, reusing the same pending id, as long as that entry hasn't been rejected) until the skill directory exists — so a "decide later" answer just means the next mention asks again. Rejecting resets the mention counter for that domain (see Reject below), so a rejected domain needs `min_mentions` fresh mentions before it re-stages, instead of asking again on the very next prompt.
4. **The staleness nudge (patch, not create).** `scripts/detect_staleness.py` catches the opposite gap: a domain that already has a skill, whose files were edited this session, but whose skill was never invoked via the `Skill` tool. It reads the whole session transcript, maps edited file paths to domains the same way `detect_domain_recurrence.py` does, and — same content-free staging — calls `pending.py new --action patch --target <domain>` when it finds one, then instructs `AskUserQuestion` at the end of the task. This is a signal the skill might be missing the workflow just used, not a signal to create a new skill.
...
- **Reject**: `python3 .claude/skills/autodidact/scripts/pending.py reject <id>` — stamps `rejected_at` (kept as history; hidden from plain `pending.py list`, visible with `--all`; cannot be approved, forces a new request). Also resets the mention counter for that domain so the recurrence hook doesn't re-stage on the very next prompt — the domain must accumulate `min_mentions` fresh mentions first.
