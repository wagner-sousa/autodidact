...
3. **The domain-recurrence nudge (self-staging, unlike the other two).** `scripts/detect_domain_recurrence.py` watches for the same uncovered domain coming up repeatedly across separate prompts — a pattern `detect_complexity.py` misses because no single turn crosses its tool-call bar. Once it fires, it stages a `create` proposal itself (a skeleton `SKILL.md` with `TODO` placeholders), so the entry lands in the pending queue without depending on the agent noticing. Your job at that point is to replace the skeleton's placeholder content with the real workflow/knowledge before approving (`pending.py show <id>` to see the staged file, overwrite it under `.state/pending/<id>/files/`, or `reject` and draft it properly later). It fires on every subsequent mention (not just the first) until the proposal exists or the skill directory does.
   A script can't write real skill content — it has no domain knowledge beyond the name. So this stays incomplete until an agent fills it in, and if the domain is never mentioned again the skeleton would otherwise be forgotten. To close that gap, `pending.py list`/`list_pending.py` (the `SessionStart` hook) flags any entry whose staged content still contains the skeleton's `TODO` marker as `[SKELETON]`, and prints an insistent reminder to replace it — every session, regardless of whether the domain comes up again.
